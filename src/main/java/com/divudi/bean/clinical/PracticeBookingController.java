...
public class PracticeBookingController implements Serializable {
    ...
    public void newOpdVisit() {
     //   System.out.println("billSession = " + billSession);
        if (billSession == null) {
         //   System.out.println("bill session is null");
            UtilityController.addErrorMessage("Please select encounter");
            opdVisit = null;
            return;
        }
     //   System.out.println("new patient encounter = ");
    ...
    public void listCompletedBillSessions() {
        String sql = "Select bs From BillSession bs where bs.retired=false and bs.serviceSession=:ss and bs.sessionDate=:ssDate and bs.patientEncounter is not null order by bs.patientEncounter.id";
        HashMap hh = new HashMap();
    ...
    public void listToCompleteBillSessions() {
        String sql = "Select bs From BillSession bs where bs.retired=false and bs.serviceSession=:ss and bs.sessionDate=:ssDate and bs.patientEncounter is null order by bs.id";
        HashMap hh = new HashMap();
        hh.put("ssDate", sessionDate);
        hh.put("ss", getSelectedServiceSession());
    ...
    public ServiceSession getSelectedServiceSession() {
        if (selectedServiceSession != null) {
            if (doctor == null) {
                ...
                if (selectedServiceSession.getStaff() == null) {
                    selectedServiceSession.setStaff(doctor);
                }

                if (!selectedServiceSession.getStaff().equals(doctor)) {
